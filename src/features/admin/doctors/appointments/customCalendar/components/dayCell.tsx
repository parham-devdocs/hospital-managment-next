...
const DayCell = ({
  ...
  const selectedTextColor = Color(dayColor).lighten(0.05).hex();
  return (
    <div className="aspect-square p-0.5 relative ">
      <button
        ...
        className={cn(
          ...
          !isSelected && [`hover:bg-[${hoverColor}]`],
          // Selected state
          isSelected && ["shadow-lg shadow-blue-500/25", "scale-105"],
          // Today state
          isToday && !isSelected && ["ring-2 ring-blue-400/40 ring-offset-2"],
          // Weekend
      ...
      >
        ...
        {/* Event Indicators */}
        <EventIndicators events={events} color={dayColor} />
      ...
      {/* Popover */}
      {hasEvents && (
        <PopoverComp date={day} appointments={events} color={dayColor} />
      )}
    </div>
  );
};
